...
The v3 gateway protocol (`docs/PROTOCOL-V3-QKD-KEYID.md`: transcript-bound
ETSI 014 `key_ID`, signed key-mode negotiation, client-signed hello) and the
relay (`src/relay.rs`) use the domain-separated, keyed key schedule specified
in `docs/KEY-SCHEDULE.md`, with known-answer vectors under `tests/vectors/`
generated by an independent implementation. The v2 schedule above is kept
