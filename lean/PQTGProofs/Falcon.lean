/-
  ...
  Falcon-512 (pending FIPS 206) parameter conformance.

  Scope: parameter sizes and number-theoretic facts for Falcon-512 as used
  by `paraxiom-pqc::sign` for handshake-transcript signatures. Does NOT
  prove EUF-CMA security; that is inherited from the Falcon submission
  (NIST PQC round-3 finalist, pending FIPS 206 standardization).

  Mirrors: src/crypto.rs (SIG_ALG = SignAlgorithm::Falcon512).
-/
