/-
  ...
  PqSession (AES-256-GCM) parameter conformance.

  Scope: byte-level structural lemmas for the session encryption layer —
  nonce framing, counter monotonicity, wire-format overhead. Does NOT prove
  AES-256-GCM IND-CCA security; that is inherited from NIST SP 800-38D.

  Mirrors: src/crypto.rs (PqSession::{encrypt, decrypt, sign_and_encrypt,
  decrypt_and_verify}; nonce = 4-byte zero prefix ‖ 8-byte u64 counter).
-/
